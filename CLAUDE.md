...
**Landmine nuevo:**
23. **iOS Safari y los form controls:** (a) un `input[type=date]`/`[time]` **vacío** colapsa su altura en iOS → mantené el `min-height` global (excluyendo checkbox/radio/range/file/color, que se deforman con altura mínima). (b) un `<button>` sin `appearance: none` toma forma nativa (`push-button`) en iOS y deforma los toggles/pills → el reset global de `button` DEBE incluir `-webkit-appearance: none`. Estos bugs NO se ven en Chromium (son de WebKit); si no hay WebKit para probar, reproducí el mecanismo (forzando `appearance:auto`) y aplicá el fix canónico.
