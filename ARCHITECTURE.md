...
> Documento generado el 2026-08-06 por un barrido exhaustivo del código (11 agentes en paralelo) + foto autoritativa de la base en producción. Es el **mapa completo de cada sección y función** de la plataforma. Complementa a `CLAUDE.md` (que es el **contrato obligatorio**: Regla de Oro, reglas no negociables, vocabulario, estética y los *landmines* numerados hasta #80). Si algo acá contradice al código actual, gana el código — actualizá este doc.

> ⚠️ **SINCRONIZACIÓN (2026-09-13).** El cuerpo de este documento es fiel al núcleo previo (~PR #105, agosto 2026): órdenes/repertorio/práctica/bandas, motor de transposición, converters/regla #8, formación/lineup, push, CI/CSP. **Lo posterior a #105 se agregó al final** en la sección **"Subsistemas Sep 2026 (sincronización)"** — leela junto con las secciones "Estado al…" de `CLAUDE.md` y la base en vivo (Supabase MCP), que son la fuente de verdad para esos temas. **Cifras del encabezado 2026-08-06 desactualizadas** — hoy en prod: **29 tablas + 2 vistas** (`members_directory`, `member_auth_orphans`), **12 crons**, **67 migraciones**, **13 Edge Functions (todas en el repo)**, ~**80 landmines**, **181 tests**. Además, tres afirmaciones del cuerpo quedaron DESACTUALIZADAS y hay que leerlas corregidas: (a) la tabla de crons lista 8 de 12 jobs y con dos nombres viejos → los correctos son `daily-afternoon-reflection` (no "daily-reflection-notification") y `notification-monitor` (no "reflection-monitor"), y faltan `send-emails-worker` / `collab-expire` / `leader-activity-digest` / `ensamble-health-report`; (b) la sección de Edge Functions dice "sólo `send-push` en el repo / admin-* desplegadas fuera del repo" → hoy las 13 EFs están en `supabase/functions/`; (c) donde dice que `notifications` "la inserta cualquier autenticado" → hoy el cliente tiene **REVOCADO** INSERT/UPDATE/DELETE sobre `notifications` (server-owned; landmine #42).
...
- Selectores Banda / Director / Tono son `<SelectMenu>` (bottom sheet en móvil, sin `<select>` nativo del navegador); director incluye la opción `{value:'', label:'Sin director'}`. **Agregar canción del repertorio** también es un `<SelectMenu searchable>` (`testId="order-song-picker"`: buscador dentro de la hoja, tono como badge, `beforeOpen` con el guard "Elegí la banda primero"; `value=""` y agrega en `onChange`) — mismo método en el modal de ministración (`ministration-picker`). Nunca un dropdown `absolute` dentro del modal (landmine #82). El tooltip de historial de tono es **por fila** (`keyHistoryTooltip.rowIndex === index`), se auto-cierra a los 6 s (`showKeyHistoryTooltip` + `keyHistoryTimerRef`, `Ordenes.jsx:982-988`) y es `pointer-events-none` (antes interceptaba taps sobre el control de director en móvil).
...
- **Formación (landmine 53):** la card del día y el `PrepBanner` (`resolveActiveOrder(orders, memberId, nowMs, isOrderParticipant)`) se muestran solo a participantes (pastor siempre). No volver a filtrar por banda efectiva o quien no toca ese día recibe "hoy tenés ensamble".
...
- **Instrumentos (registro único, seis lugares; landmine #83):** `INSTRUMENTS` (appStore) · `INSTRUMENT_ORDER` (lineup.js) ↔ `_instrument_rank` (SQL, última def `20260913_instrumento_percusion.sql`) · lista de la EF `collab` · `CHANNEL_COLORS` (channelPlan.js) · `getInstrumentIcon` (Dashboard). Actual: Voz, Coros, Guitarra Eléctrica, Guitarra Acústica, Piano, Teclado, Bajo, Batería, Percusión, Violín, Flauta, Saxofón, Trompeta.
- **`ministration.js`** — regla pura del banner "¿Con qué ministramos?" (`resolveMinistrationOrder`, ventana 3 h desde la hora del servicio, `canOfferMinistration` = pastor o líder permanente; `isMinistrationSong`). La escritura va por la RPC `add_ministration_songs` (landmine #74).
- **`bannerLifetime.js`** — VENTANAS DE VIDA de todos los banners del Inicio (fuente única, epoch UTC desde fechas ART): `rehearsalCardWindow` [día 08:00, hora del ensamble + 2 h), `prepWindow` [7 días antes, inicio del servicio), `observerChangedWindow` [cambio, inicio + 2 h), `observerServiceWindow` [día 08:00, inicio + 3 h), `observerNewWindow` [creación, +72 h), `orderChangedWindow` [cambio, min(cambio + 48 h, inicio)), `ministrationNoticeWindow` [inicio, +3 h), `collabResultWindow` [decisión, min(decisión + 48 h, inicio)); `inWindow`, `nextBoundary`, `nextArtMidnight`; resolvers `resolveMinistrationNotice` (formación/áreas que reciben la ministración; no líder/pastor) y `resolveOrderChanged` (banda efectiva ∪ pastores, excluye `contentChangedBy`). Hook `hooks/useLifetimeTick(windows)` → `now` que se re-agenda solo en el próximo límite (landmine #78). Componentes `dashboard/MinistrationNotice.jsx` y `dashboard/OrderChangedBanner.jsx`; `observerBanners.js` (`pickObserverFocus(area, orders, ctx)`, `observerWindows`) consume las mismas ventanas.
...
# Subsistemas Sep 2026 (sincronización)

> Agregado 2026-09-13 por auditoría de documentación (Workflow, 8 agentes). El cuerpo de arriba llega fiel hasta ~PR #105 (ago 2026). Acá van los subsistemas EN PROD que faltaban, con sus objetos clave y un puntero al detalle en `CLAUDE.md`. Verificado contra la base en vivo (`gvsoexomzfaimagnaqzm`) y `src/`.

## Foto de la base hoy (reemplaza el snapshot 2026-08-06)
- **29 tablas** con RLS + **2 vistas** (`members_directory`, `member_auth_orphans`). **12 crons** activos, **13 Edge Functions** (todas en `supabase/functions/`), **67 migraciones**.
- **Modelo de seguridad (3 capas):** (1) RLS apoyada en helpers SECURITY DEFINER `auth_role()` / `is_pastor()` / `is_pastor_or_leader()` / `my_member_id()` (+ INVOKER `is_sonido_observer()`, `can_open_service_presenter()`, `am_i_in_order_band()`); casi todo "SELECT = cualquier miembro activo con rol", escrituras pastor o pastor/líder. (2) GRANTs de columna que la RLS no expresa: `members` NO tiene SELECT de tabla para `authenticated` (solo columnas no-PII; el PII sale por la vista). (3) Triggers BEFORE-UPDATE "freeze" que corren como INVOKER y eximen a `service_role`.

## PII de miembros: `members` / `members_directory` / `member_private_fields` (landmine #73)
El cliente **NO lee `members` directo**: lee la **vista `members_directory`** = columnas no-PII de `members` + LATERAL `member_private_fields(id)` (SECURITY DEFINER) que devuelve `email/phone/birthdate` **solo si es la ficha propia o sos pastor**. Migración `20260913_members_revoke_select.sql`. Consecuencias: un `from('members').select('*')` da 42501; columna nueva en `members` = 3 lugares (GRANT SELECT de la columna + recrear la vista + converters). Ojo: `orders.feedback` (texto legacy) ≠ tabla `service_feedback`; `notifications.is_read` (bool legacy) ≠ tabla `notifications_read` (estado de lectura por usuario, el modelo vigente).

## Triggers anti-escalada (freeze de columnas privilegiadas)
- `enforce_member_update_rules` (INVOKER, landmine #41): un no-pastor no puede cambiar `role/editor/active/user_id/email/areas` de una ficha. DEBE seguir INVOKER (un DEFINER bypassa RLS).
- `enforce_band_update_rules`: un líder solo puede AGREGAR miembros a `bands.members` (no quitarse/agregarse a sí mismo ni tocar otros campos).
- `notifications`: INSERT/UPDATE/DELETE **REVOCADOS** al cliente (solo SELECT) — no puede forjar notificaciones/spam push (landmine #42). Las filas las crean triggers `notify_on_*` / EFs.
- Otros: `enforce_member_auth_link` (rol pastor/leader exige `user_id`), `enforce_no_duplicate_active_temp` (temporales sin solaparse), `force_author_created_by/updated_by` (autoría firmada por el servidor).

## Pipeline de correo (Gmail transaccional)
El corazón asíncrono. `encolar_email(slug, destinatario, vars)` → fila en **`email_queue`** → cron **`send-emails-worker`** (`* * * * *`, jobid 14) → `trigger_send_emails()` → EF **`send-emails`** (lee secretos con `get_email_config()`, renderiza `email_templates` con `cuerpo_html` RAW + `_html_escape` de todo dato de usuario, manda por Gmail API) → registra `sent_emails` + `email_throttle`. Editor de plantillas para pastores (PR #63). `send-push` es el análogo para web-push. **QA: pausar el worker para no spamear a los 8 usuarios reales.**

## Multi-área: observadores Multimedia/Sonido (PR #108/#109/#119/#136)
`members.areas text[]` (freezeado por `enforce_member_update_rules`; solo pastor/EF; espejo `src/lib/areas.js`). Capacidades por área en **un** registro de helpers SQL `_area_email_slugs()` / `_area_formation_slugs()` (→ {multimedia,sonido}) / `_area_presenter_slugs()` + `can_open_service_presenter()` (relaja el presentador para observadores) + `is_sonido_observer()`. Los observadores reciben correos por área ({{ensamble}}/{{area_nota}}) y, en cambios de formación, aviso a ambas áreas. Banners del inicio: `ObserverAreaBanners.jsx` + `src/lib/observerBanners.js`. `pastor_area` ≠ área. **El pastor es multiárea por ROL** (landmine #76). Detalle: CLAUDE.md "Estado al 2026-09-11 (III)" y "(2026-09-12)".

## Suspender / Reactivar / Reprogramar ensamble (PR #121; landmines #64/#66)
Columnas server-owned en `orders`: `rehearsal_suspended_at/reason/by` (van en `convertOrderFromDB`, NUNCA en ToDB). Se escriben SOLO por las RPCs **`suspend_order_rehearsal` / `resume_order_rehearsal` / `reschedule_order_rehearsal`** (SECURITY DEFINER, CAS atómico anti-doble-aviso, gate `_can_manage_order_rehearsal` = pastor o líder miembro permanente). Trigger INVOKER **`enforce_order_rehearsal_rules`** congela esas columnas para el cliente y limpia la suspensión al reprogramar. Fan-out por `_fanout_rehearsal_event` a formación ∪ pastores ∪ **Sonido**. El cron `rehearsal-reminders` filtra `status='scheduled' AND rehearsal_suspended_at IS NULL`. 3 plantillas `ensamble-suspendido/reactivado/reprogramado`. Cliente: `RehearsalActionModal.jsx`, store `suspend/resume/rescheduleRehearsal`.

## Salud de los ensambles: reporte trimestral (PR #122; landmine #65)
`send_ensamble_health_report()` (SECURITY DEFINER service_role-only), cron **`ensamble-health-report`** (`0 11 * * *`, jobid 17) auto-gateado cada 90 días desde ancla 2026-10-01 (primer envío ~30/12). "Concretado" = `rehearsal_date` seteado AND no suspendido. % por banda + frase por tramo + versículo.

## Esquema de reunión + "Iniciar servicio" (presentador) (PR #91)
Tablas `service_schemas` (run-of-show por orden: secciones jsonb; en "Adoración" asigna canciones del orden) y `schema_templates` (plantillas del pastor). RLS por `am_i_in_order_band()` + `can_open_service_presenter()`. Cliente: `SchemaBuilderModal.jsx`, `src/lib/serviceSchema.js`, y la página **`IniciarServicio.jsx` en la ruta `/servicio/:orderId` FUERA del `Layout`** (presentador fullscreen por dispositivo: sección + cuenta regresiva + Play/Stop/Siguiente; en Adoración una canción por vez con acordes en el tono + autoscroll). Tiene su propio guard e `initialize`.

## Solicitar colaboración (PR #88; landmine #43)
Tablas `collaboration_requests` / `collaboration_participants` (cliente **solo SELECT**; DML revocado). 5 RPCs `collab_*` (SECURITY DEFINER) invocadas **solo por la EF `collab`** (service_role), no por el cliente. Cron `collab-expire` (`0 7 * * *`). Plantillas `colaboracion-*`. Elegibles (activos con la categoría, fuera de la banda) reciben correo+push+banner "Yo me ofrezco"; al cubrir, el elegido entra como temporal. Recursión RLS cruzada resuelta con helpers DEFINER (landmine #43). Cliente: `CollaborationBanner`.

## "¿Con qué ministramos?" — canción de ministración (PR #135; landmine #74)
Se escribe SOLO por la RPC **`add_ministration_songs`** (gate pastor/líder-en-banda, ventana [inicio, +3 h), idempotente). Marca = clave `ministracion:true` en `orders.songs[]` (sin `_`, la conserva `stripSongRefs`). Avisa a formación ∪ pastores ∪ áreas y **silencia el aviso genérico** ocupando el throttle `order_edit:<id>`. Migración `20260913_ministracion_y_ensayometro.sql`. Cliente: `src/lib/ministration.js`, store `addMinistrationSongs`, `MinistrationBanner`/`MinistrationNotice`.

## Feedback post-servicio "¿Cómo estuvimos?" (PR #71/#132; landmine #68)
Tabla **`service_feedback` write-only-via-EF** (cliente solo SELECT, sin policy de INSERT). EF **`send-service-feedback`** (gate pastor o líder de la banda, ventana 48 h desde la hora del servicio). Regla espejada en `src/lib/serviceFeedback.js`. Plantilla `feedback-post-servicio`. Distinto del `orders.feedback` legacy.

## `member_activity` — ficha de actividad (solo pastor) (PR #85)
Tabla con telemetría (`last_seen_at`, `app_installed_at`, `notifications_on`). SELECT **solo pastor**; la escribe la RPC `record_member_activity()` (upsert para `auth.uid()`). Trigger `sync_member_notifications_flag` (sobre `push_subscriptions`) mantiene `members.notifications_enabled`. Panel en `Miembros.jsx`.

## `order_channel_plans` — plan de canales de Sonido (PR #119)
Tabla del plan de micrófonos/canales por orden. CRUD por pastor/líder **y observador de Sonido** (`is_sonido_observer`), autoría por `force_author_updated_by`. Cliente: `ChannelPlanModal.jsx`, `src/lib/channelPlan.js` (arma filas por (miembro, instrumento) desde la formación — un miembro sin instrumento no genera fila).

## Sellos server-owned de `orders`: `content_changed_at/by` (landmine #78)
Trigger BEFORE `set_order_content_changed` sella `content_changed_at` + `content_changed_by` (`my_member_id()`) cuando cambia contenido o formación, y **fuerza ambos a OLD** si no hubo cambio real (un PATCH del cliente no puede inventarlos). Van en `convertOrderFromDB`, NUNCA en ToDB. Alimentan el banner "Hubo cambios" del inicio (excluye al que hizo el cambio). Migración `20260913_order_content_changed_by.sql`.
...
- **Arranque:** `src/lib/boot.js` `runBoot` = `restoreSession()` → `Promise.all([bootProfile, initializeApp])` → `markRefreshed()` (paralelo ficha+tablas DESPUÉS de la sesión; landmines #61/#62). `initialize()` carga **9 tablas** (members/bands/songs/orders + band_temporary_members + collaboration_requests/_participants + service_schemas + schema_templates). `lazyPage` de `App.jsx` va envuelto en `withChunkRecovery` (recuperación de chunks viejos tras deploy; landmine #67).
- **Banners del Inicio:** el Dashboard hospeda TODOS con ventana de vida en `src/lib/bannerLifetime.js` + hook `useLifetimeTick` (aparecen/desaparecen solos; landmine #78): `ServiceFeedbackPrompt`, `MinistrationBanner`/`MinistrationNotice`, `CollaborationBanner`, `OrderChangedBanner`, `ObserverAreaBanners`, cumpleaños, preparación, ensamble.
- **Libs puras nuevas** (encodan reglas de producto, espejo manual del SQL): `areas.js`, `serviceSchema.js`, `serviceFeedback.js`, `channelPlan.js`, `ministration.js`, `bannerLifetime.js`, `observerBanners.js`, `chunkRecovery.js` (#67), `boot.js`, `updateProgress.js` (#60), `searchText.js` (buscador único, #77), `repertoireInsights.js` (Radiografía), `ensayometro.js` (espejo de `send_practice_reminders`, singer-only, #75), `lyricsDocx.js`.
- **Store nuevo:** además del CRUD, `suspend/resume/rescheduleRehearsal`, `addMinistrationSongs`, `refetchOrder`, acciones de colaboración / esquema / plan de canales / miembros temporales.
- **Comunicaciones:** broadcast del pastor por EF `admin-send-communication` → `communications` (saneado, `sender_id`) + `communication_notifications` + push; editor contentEditable rico (#68).

## Digest y otros crons (tabla completa: ver CLAUDE.md "Crons activos")
`leader-activity-digest` (`0 1 * * *`, jobid 16): resumen diario a pastores (solo correo) de altas/ediciones/eliminaciones de líderes/editores; clasifica por los campos que cambiaron de verdad (`activity_digest_items`, landmine #51 — excluir columnas "automáticas": `updated_at`, `rehearsal_reminder_sent`, `content_changed_*`, `last_seen_at`).
