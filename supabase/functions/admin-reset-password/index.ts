...
async function requirePastor(req: Request) {
  ...
  const { data: caller, error: cErr } = await admin.from("members").select("role, active").eq("user_id", user.id).maybeSingle();
...
Deno.serve(async (req: Request) => {
  ...
  if (!newPassword || typeof newPassword !== "string" || newPassword.length < 6) {
    return json({ error: "newPassword required (min 6 chars)" }, 400);
  }

  const { error: updErr } = await admin.auth.admin.updateUserById(userId, { password: newPassword });
  if (updErr) return json({ error: "password update failed", detail: updErr.message }, 400);
