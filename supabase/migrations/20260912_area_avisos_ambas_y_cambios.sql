-- ============================================================================
-- Avisos de cambios de orden/formación a AMBAS áreas (Multimedia + Sonido) por
-- push + mail, y marca content_changed_at para el banner "¡Ojo! Hubo cambios".
-- ============================================================================

-- 1) Columna sello: la escribe SOLO la base (trigger). NUNCA el cliente (no va en
--    convertOrderToDB, mismo patrón que rehearsal_reminder_sent).
...
AS $function$
BEGIN
  -- Solo cuando cambia el CONTENIDO o la FORMACIÓN (no status/rehearsal_reminder_sent/etc.).
...
DROP TRIGGER IF EXISTS set_order_content_changed ON public.orders;
CREATE TRIGGER set_order_content_changed
  BEFORE UPDATE ON public.orders
  FOR EACH ROW EXECUTE FUNCTION public.set_order_content_changed();

-- 2) La formación ahora avisa a AMBAS áreas observadoras (antes solo Sonido).
...
-- 3) notify_on_order_update: observadores reciben PUSH además del mail en cambios de
--    contenido; y en el cambio SOLO de formación se avisa (push + mail) a ambas áreas.
...
BEGIN
  ...
  BEGIN
    ...
    IF v_content THEN
      ...
      LOOP
        ...
        v_area_nota := CASE WHEN (NOT v_is_core) AND v_obs IS NOT NULL
          ...
          ELSE '' END;
        BEGIN
          -- Campanita (push) a TODOS los destinatarios con cuenta: banda, pastores Y
          -- observadores de área (Paul: los cambios avisan por push + mail a las áreas).
    ...
    ELSE
      ...
      -- Aviso a observadores de área (Multimedia + Sonido) por PUSH + MAIL cuando cambia
      -- SOLO la formación, excluyendo a quien ya recibió el aviso personal.
      FOR v_member IN
        SELECT DISTINCT m.id, m.name, m.email, m.user_id FROM public.members m
        WHERE m.active = true AND m.user_id IS DISTINCT FROM v_uid
