...
2. Trigger `BEFORE INSERT OR UPDATE OF lineup` **`validate_order_lineup`** (⚠️ CORRECCIÓN 2026-09-13: se implementó como **SECURITY DEFINER**, no INVOKER — es load-bearing, ver landmine #54: como INVOKER un líder vería menos filas y el validador filtraría participantes legítimos en silencio): normaliza y valida — `mode` ∈ {all, custom}; `members[].memberId` ⊆ `band_effective_member_ids(NEW.band_id)` (si no → `RAISE` P0001 con mensaje claro); `instruments` ⊆ `members.instruments` del miembro (los que no correspondan se descartan); sin duplicados; `definedBy` = `my_member_id()` cuando hay JWT (nunca confiar en el cliente); `definedAt = now()`. Si `mode='all'` → `members := []`. Cambiar `band_id` con `custom` → el trigger filtra los que ya no están en la banda nueva (y avisa vía `RAISE NOTICE`; el cliente muestra el ajuste).
